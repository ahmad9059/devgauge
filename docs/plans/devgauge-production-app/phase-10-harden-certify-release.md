...
- **Hetzner VPS hardening:** SSH key-only access, `ufw`/no public inbound ports (or Cloudflare Tunnel), non-root deploy user, container resource limits and read-only roots, unattended security updates, `fail2ban`, and disposable/stateless recovery (re-provision + redeploy). Verify the VPS never holds secrets in plaintext (envelope encryption).
- Run automated SAST, dependency/container scans, SBOM policy, secret scan, infrastructure policy checks, API DAST, and Android binary review.
...
- Profile low/mid-range Android hardware on real devices; eliminate main-thread stalls and unnecessary background work.
...
- Re-run the complete Reset Horizon state catalogue on Android phones plus supported tablet/expanded, split/multi-window, continuous-resize, and foldable posture classes in Dark and Light.
- Capture native emulator/device evidence for all required routes and material states; hardware checks cover gestures, haptics, performance, notification behavior, and system integration.
- Audit TalkBack, switch/keyboard access where applicable, large text/font scale, in-app text-size choices, both themes, bold/increased contrast, reduced motion, color filters, landscape, safe areas, IME, continuous resize, hinge occlusion, and predictive/gesture back.
...
- Complete Android permissions/data-safety declarations and F-Droid metadata (no Google Play Services dependency, no proprietary trackers); remove unnecessary capabilities.
...
- Promote the Phase 2 staging blueprint into a separate staging box (or an isolated namespace on the Hetzner VPS) with private networking, Neon branch, Upstash instance, R2 bucket, and least-privilege identities; verify no staging identity/key/data crosses the production boundary.
...
- [ ] Backup and deletion-ledger expiration jobs meet the approved 14/30-day policy and produce non-sensitive compliance evidence.
...
- [ ] Core journey E2E tests pass on supported Android versions, including background/cold-start notification routes.
- [ ] TalkBack, large text, in-app text sizing, Dark/Light/System, increased contrast, reduced motion, color filters, orientation, safe areas, split/multi-window, continuous resize, foldable posture, and native back behavior pass.
...
- [ ] Hetzner VPS hardening (SSH keys only, no public inbound ports or Cloudflare Tunnel, non-root deploy, unattended updates) is verified; a re-provision + redeploy recovery drill passes.
